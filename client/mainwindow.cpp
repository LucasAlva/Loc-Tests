    ...
    ui(new Ui::MainWindow)
{
    ...
    QObject::connect(ui->lineEdit_ExcavationTranslation, &IntEdit::valueEdited, this, &MainWindow::handleExcavationTranslationSet); //so that we can send values outside the range of the slider
    QObject::connect(ui->slider_ExcavationTranslation, &QSlider::valueChanged, this, &MainWindow::handleExcavationTranslationSet);
    ...
    connect(ui->checkBox_RunCloseLoop, &QCheckBox::stateChanged, this, &MainWindow::handleClosedLoop);

    connect(ui->pushButton_InitialExcavationTranslation, &QPushButton::clicked, this, &MainWindow::handleInitialExtend);
    connect(ui->pushButton_MaxExcavationArm, &QPushButton::clicked, this, &MainWindow::handleMaxExcavationArm);
    connect(ui->pushButton_MidExcavationArm, &QPushButton::clicked, this, &MainWindow::handleMidExcavationArm);
    connect(ui->pushButton_MinExcavationArm, &QPushButton::clicked, this, &MainWindow::handleMinExcavationArm);
...
void MainWindow::handleLocomotionUp() {
    ...
    AllWheelsCommand msg;
    msg.set_front_left(ui->slider_LocomotionSpeed->value());
    msg.set_front_right(ui->slider_LocomotionSpeed->value());
    msg.set_back_left(ui->slider_LocomotionSpeed->value());
    msg.set_back_right(ui->slider_LocomotionSpeed->value());
    ...
    ex->Publish((char*)msg_buff, msg_size, "motorcontrol.locomotion.all.all_wheels");
...
void MainWindow::handleLocomotionDown() {
    ...
    AllWheelsCommand msg;
    msg.set_front_left((-1 * (ui->slider_LocomotionSpeed->value())));
    msg.set_front_right((-1 * (ui->slider_LocomotionSpeed->value())));
    msg.set_back_left((-1 * (ui->slider_LocomotionSpeed->value())));
    msg.set_back_right((-1 * (ui->slider_LocomotionSpeed->value())));
    int msg_size = msg.ByteSize();
    void *msg_buff = malloc(msg_size);
    if (!msg_buff) {
        ui->consoleOutputTextBrowser->append("Failed to allocate message buffer.\nDetails: malloc(msg_size) returned: NULL\n");
...
void MainWindow::handleLocomotionStop() {
    ...
    AllWheelsCommand msg;
    msg.set_front_left(0);
    msg.set_front_right(0);
    msg.set_back_left(0);
    msg.set_back_right(0);
    int msg_size = msg.ByteSize();
    void *msg_buff = malloc(msg_size);
    if (!msg_buff) {
        ui->consoleOutputTextBrowser->append("Failed to allocate message buffer.\nDetails: malloc(msg_size) returned: NULL\n");
...
void MainWindow::handleExcavationTranslationSet(int value) {
    ...
    AMQPExchange * ex = m_amqp->createExchange("amq.topic");
    ex->Declare("amq.topic", "topic", AMQP_DURABLE);
    ex->Publish((char*)msg_buff, msg_size, "motorcontrol.excavation.conveyor_translation_speed");

    free(msg_buff);

    //ui->lineEdit_ExcavationTranslation->setText(QString::number(value));
    //QString val = QString::number(value);
    //ui->consoleOutputTextBrowser->append(val);
    ui->slider_ExcavationTranslation->setValue(value);

    ui->lcdNumber_ExcavationTranslation->display(value);
}

void MainWindow::handleExcavationTranslationExtend() {
    ui->slider_ExcavationTranslation->setValue(35); //100 all the way down
}
...
void MainWindow::handleExcavationTranslationRetract() {
    ui->slider_ExcavationTranslation->setValue(-35); //0 all the way up
}
...
void MainWindow::initSubscription() {
    ...
    msg.set_loc_obs_detailed(false);
...
void MainWindow::handleState(QString key, QByteArray data) {
    ...
    float fl_rpm = s.loc_detailed().front_left_rpm();
    float fr_rpm = s.loc_detailed().front_right_rpm();
    float bl_rpm = s.loc_detailed().back_left_rpm();
    float br_rpm = s.loc_detailed().back_right_rpm();
    float speed = s.loc_summary().speed();
    float arm_pos = s.exc_summary().arm_pos();
    float translation_pos = s.exc_summary().displacement();
    //float bc_current = s.excdetailed().conveyor_motor_current();
    bool exc_ext_left = s.exc_detailed().translation_left_extended();
    bool exc_ext_right = s.exc_detailed().translation_right_extended();
    bool exc_ret = s.exc_detailed().translation_retracted();
    bool exc_dig_left = s.exc_detailed().arm_left_extended();
    bool exc_dig__right = s.exc_detailed().arm_right_extended();
    bool dep_ext_left = s.dep_detailed().dump_left_extended();
    bool dep_ext_right = s.dep_detailed().dump_right_extended();
    bool dep_ret_left = s.dep_detailed().dump_left_retracted();
    bool dep_ret_right = s.dep_detailed().dump_right_retracted();
    float load_cell_left = s.dep_detailed().left_load();
    float load_cell_right = s.dep_detailed().right_load();
    ...
    ui->ledIndicator_ExcavationTranslationRetract->setState(exc_ret);
    ui->ledIndicator_ExcavationArmExtendLeft->setState(exc_dig_left);
    ui->ledIndicator_ExcavationArmExtendRight->setState(exc_dig__right);
    ui->ledIndicator_DepositionExtendLeft->setState(dep_ext_left);
    ui->ledIndicator_DepositionExtendRIght->setState(dep_ext_right);
    ui->ledIndicator_DepositionRetractLeft->setState(dep_ret_left);
    ui->ledIndicator_DepositionRetractRight->setState(dep_ret_right);
    ui->speedometer_Excavation->setSpeed(translation_pos);
    ui->lcdNumber_LoadCellLeft->display(load_cell_left);
    ui->lcdNumber_LoadCellRight->display(load_cell_right);
...
handleTankPivotR() {
    if (0 == m_desiredConfig) { // straight
        ...
        AllWheelsCommand msg;
        msg.set_front_left(leftSide);
        msg.set_front_right(rightSide);
        msg.set_back_left(leftSide);
        msg.set_back_right(rightSide);
        int msg_size = msg.ByteSize();
        void *msg_buff = malloc(msg_size);
        if (!msg_buff) {
            ui->consoleOutputTextBrowser->append("Failed to allocate message buffer.\nDetails: malloc(msg_size) returned: NULL\n");
...
void MainWindow::handleTankPivotLB() {
    if (0 == m_desiredConfig) { // straight
...
void MainWindow::handleTankPivotRB() {
    if (0 == m_desiredConfig) { // straight
        int rightSide = (ui->slider_LocomotionSpeed->value()* (-1)); //right wheel speed
        int leftSide = (ui->slider_UpsetSpeed->value()* (-1)); //left wheel speed
        QString r = QString::number(leftSide);
        QString l = QString::number(rightSide);
        ui->consoleOutputTextBrowser->append("Right Side: " + r + "Left Side: " + l);
...
void MainWindow::handleTankPivotRK() {
    if (0 == m_desiredConfig) { // straight
        ...
        int rightSide = (ui->slider_LocomotionSpeed->value() * (-1)); //left wheel speed * (-0.5)
        QString r = QString::number(leftSide);
        QString l = QString::number(rightSide);
        ui->consoleOutputTextBrowser->append("Right Side: " + r + "Left Side: " + l);
        /*handleFrontRightWheelSet(rightSide);
...
void MainWindow::handleTankPivotLK() {
    if (0 == m_desiredConfig) { // straight
        int leftSide = (ui->slider_LocomotionSpeed->value() * (-1)); //left wheel speed * (-0.5)
        int rightSide = (ui->slider_LocomotionSpeed->value()); //right wheel speed
        QString r = QString::number(leftSide);
        QString l = QString::number(rightSide);
        ui->consoleOutputTextBrowser->append("Right Side: " + r + "Left Side: " + l);
...
//ease of access positions
void MainWindow::handleInitialExtend() {
    handleExcavationTranslationSet(50);
    sleep(1.5);
    handleExcavationTranslationSet(0);

    AutonomyNextHeading msg;
    msg.set_heading(25.0F);
    int msg_size = msg.ByteSize();
    void *msg_buff = malloc(msg_size);
    if (!msg_buff) {
        ui->consoleOutputTextBrowser->append("Failed to allocate message buffer.\nDetails: malloc(msg_size) returned: NULL\n");
    ...
    QString h = QString::number(msg.heading());
    msg.SerializeToArray(msg_buff, msg_size);

    AMQPExchange * ex = m_amqp->createExchange("amq.topic");
    ui->consoleOutputTextBrowser->append("Sending Next heading " + h + " ");
    ex->Declare("amq.topic", "topic", AMQP_DURABLE);
    ex->Publish((char*)msg_buff, msg_size, "heading");

    free(msg_buff);
}

void MainWindow::handleMaxExcavationArm() {
    handleExcavationArmSet(100);
}

void MainWindow::handleMidExcavationArm() {
    handleExcavationArmSet(50);
}

void MainWindow::handleMinExcavationArm() {
    handleExcavationArmSet(-50);
}

void MainWindow::handleClosedLoop(bool checked) {
    if(checked) {
        ui->consoleOutputTextBrowser->append("Runing Motors in Closed Loop");
    }
    if(!checked) {
        ui->consoleOutputTextBrowser->append("Stopped Runing Motors in Closed Loop");
    }
    ClosedLoopCommand msg;
    msg.set_closed(checked);
    int msg_size = msg.ByteSize();
    void *msg_buff = malloc(msg_size);
    if (!msg_buff) {
        ui->consoleOutputTextBrowser->append("Failed to allocate message buffer.\nDetails: malloc(msg_size) returned: NULL\n");
    ...
    AMQPExchange * ex = m_amqp->createExchange("amq.topic");
    ex->Declare("amq.topic", "topic", AMQP_DURABLE);
    ex->Publish((char*)msg_buff, msg_size, "motorcontrol.locomotion.all.closed_loop");
